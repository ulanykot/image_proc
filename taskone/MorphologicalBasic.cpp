...
void MorphologicalBasic::hitOrMiss(cimg_library::CImg<unsigned char> &image, const std::array<std::array<int, 3>, 3> &se) {
    cimg_library::CImg<unsigned char> filteredImage = image;
...
void MorphologicalBasic::doHMT(cimg_library::CImg<unsigned char> &image, int se) {
    hitOrMiss(image,assignNumberToSEHMT(se));
}
